...
<Note>In the above example, the Parent component passes a changing `name` property to the Child component. The Child component will only re-render when the count signal changes.</Note>
...
Qwik also provides the experimental [`Show`](/docs/labs/show/) component for QRL-based
conditional control flow once it is enabled with `experimental: ['show']`.
...
<Note>The API does not work with `useStore` since it does not return a signal. You can still use the manual approach, combining value and onInput$ as shown below.</Note>
...
<Note>This API ensures that the `value` of the input is always in sync with the value of the signal, no matter where the change comes from.</Note>
...
## Rendering Work That Waits

Sometimes part of the UI needs to wait before it can show content, such as showing a fallback while a profile card, search result, or other section is still being prepared.

Wrap that part of the UI in [`<Suspense>`](/docs/core/suspense/) when you want a fallback boundary:
...
`<Suspense>` does not define the work. It only controls what renders while that part of the UI waits.
